...
import {
  LayoutDashboard, User, Users, GitBranch, Star, Briefcase,
  MessageSquare, CalendarDays, Sun, Moon, LogOut, GraduationCap, Menu, X
} from 'lucide-react'
...
const navItems = [
  ...
  { to: '/student/alumni', icon: Users, label: 'Discover Alumni' },
  { to: '/student/connections', icon: GitBranch, label: 'Connections' },
  { to: '/student/mentorship', icon: Star, label: 'Mentorship' },
  { to: '/student/referrals', icon: Briefcase, label: 'Referrals' },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-slate-900 border-r border-slate-800 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-800">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-400">Student Portal</p>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {navItems.map(({ to, icon: Icon, label, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `nav-link-base ${isActive ? 'active-student' : ''}`
              }
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 bg-slate-900/80 backdrop-blur border-b border-slate-800 flex items-center px-4 gap-3">
          ...
          <h1 className="text-sm font-semibold text-slate-200">Student Dashboard</h1>
        </header>
